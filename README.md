...
- 친구 방: 5자리 초대 코드로 최대 4명 참가, 방장이 어절 수·제한 시간·3~10라운드를 설정하고 게스트 준비 후 직접 시작
- 경쟁: 1인당 2회의 `압박 -2초` 카드, 중간 돌발 이벤트, 상대 진행률과 누적 점수
- 대기실: 참가자 머리 위 말풍선 채팅, 방장 전용 규칙 UI, 방장 이탈 시 방 자동 파괴
...
- 10문항 합산과 최종 등급 결과
- 최대 4인 매칭, 초대 코드, 방장 시작, 라운드 규칙, 말풍선 채팅, 압박 카드와 최종 순위
